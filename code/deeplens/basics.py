...
DEFAULT_WAVE = 0.525 
# WAVE_RGB = [0.656, 0.588, 0.486] # F,D1+D2,C in Fraunhofer lines
WAVE_RGB = [0.640, 0.525, 0.470] # corresponding to GS3-U3-41C6C-C
...
MATERIAL_TABLE = { 
    ...
    "air":          [1.,       math.inf],
    "occluder":     [1.,       math.inf],
    "hexadecane":   [1.4329,   math.inf],
    "tridecane":    [1.4256,   math.inf],
    "fused-silica": [1.4585,   67.82], # https://refractiveindex.info/?shelf=glass&book=fused_silica&page=Malitson
    "f2":           [1.620,    36.37],
    "f5":           [1.6034,   38.03],
    "bk1":          [1.5101,   63.47],
    "bk7":          [1.5168,   64.17],
    ...
    # [DOI: 10.1117/1.OE.61.6.065103]
    "ol1224-vis":   [1.2912,   108.49],
}
...
SELLMEIER_TABLE = {
    ...
    "fused-silica": [0.6961663, 0.0684043**2, 0.4079426, 0.1162414**2, 0.8974794, 9.896161**2], # https://refractiveindex.info/?shelf=glass&book=fused_silica&page=Malitson
    ...
    # optotune
    'ol1224-vis': [-57989.9304, 0.0226608652, 57990.5863, 0.0226606879, 14395.0935, 6619519.09],
...
class Ray(DeepObj):
    def __init__(self, o, d, wvln=DEFAULT_WAVE, coherent=False, device=DEVICE):
        ...
        Args:
            o (torch.Tensor): ray position. shape [..., 3]
            d (torch.Tensor): normalized ray direction. shape [..., 3]
            wvln (float, optional): wvln. Defaults to DEFAULT_WAVE.
            ra (torch.Tensor, optional): Validity. Defaults to None.
            en (torch.Tensor, optional): Spherical wave energy decay. Defaults to None.
            obliq (torch.Tensor, optional): Obliquity energy decay, now only used to record refractive angle. Defaults to None.
            opl (torch.Tensor, optional): Optical path length, Now used as the phase term. Defaults to None.
        ...
        self.trace = [] # record ray tracing history
                
        self.to(device)
        self.d = nnF.normalize(self.d, p=2, dim=-1)
    
    def log_trace(self):
        """ Log the ray tracing history.
        
            This function is called in the trace function if record is required.
        """
        ray_o = self.o.clone().detach()
        # for invalid rays, we log the trace with nan value
        ray_o[self.ra == 0] = float('nan')
        
        self.trace.append(ray_o)
    ...
    def march_t(self,t,n=1):
        """ March the ray by a given time step.
    ...
    def propagate_to(self, z, n=1):
        """ Ray propagates to a given depth. works for both directions
...
# ----------------------------------------------------------------------------------------
class CRF():
    '''Camera response function, currently is threshold linear to map to the camera range
        i.e. a linear mapping from range [0,1) to [min,max)
    '''
    def __init__(self,min=0.,max=1.) -> None:
        if type(min) == float:
            self.min = torch.tensor([min, min, min])
        elif type(min) == list:
            self.min = torch.tensor(min)
        else:
            raise Exception("min should be a float or a list of 3 floats")
        
        if type(max) == float:
            self.max = torch.tensor([max, max, max])
        elif type(max) == list:
            self.max = torch.tensor(max)
        else:
            raise Exception("max should be a float or a list of 3 floats")
        
        self.min = self.min.unsqueeze(0).unsqueeze(-1).unsqueeze(-1) # [1,3,1,1]
        self.max = self.max.unsqueeze(0).unsqueeze(-1).unsqueeze(-1) # [1,3,1,1]
    ...
    def get_optimizer_params(self,lr):
        params = []
        self.min.requires_grad, self.max.requires_grad = True,True
        params.append({'params': [self.max], 'lr': lr["CRF"]})
        params.append({'params': [self.min], 'lr': lr["CRF"]})
        return params

    def __call__(self, x):
        assert x.dim() == 4, "Input should be of shape [B,3,H,W]"
        assert x.shape[1] == 3, "Input should be of shape [B,3,H,W]"
        
        out = x * (self.max - self.min) + self.min
